...
##
##wx.BitmapFromBufferRGBA(width, height, dataBuffer): Creates
##the bitmap from a buffer containing RGBA bytes.

#_dc.SetPen(_wx.Pen('black', 1))
#_dc.DrawText(text, x, y)
#_dc.DrawLine(1, 2, 2, 2)
...
##print(arr)
##print('------------------------------------')
###_bitmap = _wx.EmptyBitmap(4, 4, 32)
##_bitmap = _wx.BitmapFromBufferRGBA(4, 4, arr)
##_bitmap.CopyToBuffer(arr, format=_wx.BitmapBufferFormat_RGBA)
##print(arr)
##print('------------------------------------')
##_dc = _wx.MemoryDC()
##_dc.SelectObject(_bitmap)
##_dc.SetBrush(_wx.Brush('#ff0000'))
##_dc.DrawRectangle(0, 0, 3, 3)
...
## mouse event codes: http://docs.wxwidgets.org/2.8.4/wx_wxmouseevent.html#wxmouseevent
    ...
    def _OnLeftMouseUp(self, evt):
        self._leftdown = 0
        lock = 0
        if self._zooming or self._spinning:
            if self._zooming:
                if self._rightdown:
                    self._spinning = 1
                    lock = 1
