...
class display(cvisual.display_kernel):
    ...
    def __init__(self, **keywords):
        ...
        self.standard = (self.window == None) # standard means this display makes the window
    ...
    def bind(self, namestring, fn, **kw):
        """
        ...
        for name in namestring.split():
            ...
            elist.append((enabled, fn, kw))  # each entry is a list [enabled, fn]
    ...
    def unbind(self, namestring, in_fn=None):
        ...
        new_list = []
        for name in namestring.split():
            for enabled, fn, kw in self._bindings.get(name,[]):
                if in_fn is not fn:
                    new_list.append((enabled, fn, kw))
    ...
    def _enable_binding(self, name, in_fn, enable):
        ...
        for enabled, fn, kw in self._bindings.get(name,[]):
            if in_fn is fn:
                new_list.append((enable, fn, kw))
            else:
                new_list.append((enabled, fn, kw))
                
        self._bindings[name] = new_list
                
    def _dispatch_event(self, name, evt=None):
        for enabled, fn, kw in self._bindings.get(name,[]):
            if enabled:
                newKW = {}   # potential keywords for the bound function
                args = getargspec(fn)
                if evt is not None:
                    newKW.update(evt)
                if kw is not None:
                ...
                if 'name' in args.args:
                    newKW.update({'name':name})
                    
                applyKW = {}  # the keywords to actually apply to the bound function
                
                [applyKW.update({name:newKW.get(name,None)}) for name in args.args if name is not 'self'] # populate the available keywords
                apply(fn, (), applyKW)  # call the function!

    def trigger(self, namestring, **kw):
        """
        trigger arbitrary events with keywords...
        """
        for name in namestring.split():
            self._dispatch_event(name, kw)
    ...
    def _make_canvas(self, parent):
        ...
        w = self._width-_dw
        h = self._height-_dh
        if not self.standard: # place canvas in existing window
            x = self._x
            y = self._y
            w = self._width
            h = self.height
...
##        print(evt.GetWheelRotation(), evt.GetWheelDelta())
    ...
    def _OnMouseMotion(self, evt):
        self._report_mouse_state(evt)
        self._dispatch_event('mousemove', {'mouse':self.mouse})
        evt.Skip() # to permit setting focus
...
# CMD  + 1-button mouse = CTRL and CMD, left button
    ...
    def _report_mouse_state(self, evt): # wx gives x,y relative to upper left corner
        ...
        if lock and not self._captured:
            ...
            self.win.SetCursor(_wx.StockCursor(_wx.CURSOR_ARROW))
...
##                left = 0
    ...
    def _OnCharEvent(self, evt):
        ...
        self.pushkey(k)
        self._dispatch_event('char', {'key':k})
        evt.Skip()
#
# The wx key codes:
#
...
#
    ...
    def _OnKeyDown(self, evt):
        ...
        self._dispatch_event('keydown', {'key':k})
        evt.Skip()
        
    def _OnKeyUp(self, evt):
        k = self._ProcessKey(evt)    
        self._dispatch_event('keyup', {'key':k})
        evt.Skip()
...
# This is an atexit handler that's installed if a display is activated, so that
# programs remain 'running' after they've finished executing the code in the
# program body. For visual this is important so that all the windows don't close
    ...
    for d in displays:
        ...
        if _mouse_binding_names.intersection(events):
            ...
            if d.mouse.events:
                ...
                if event.press:
                    d._dispatch_event("mousedown", {'mouse':event})
                elif event.release:
                    d._dispatch_event("mouseup", {'mouse':event})
                if event.click:
                    d._dispatch_event("click", {'mouse':event})
...
if _plat == 'Macintosh':
    ...
    _dw = 0
    _dh = _dy = 52 # ad hoc Xubuntu; not fully understood
else: # Windows
    _dw = 2*(_wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_X))
    _dh = 2*(_wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_Y)) + \
            _wx.SystemSettings.GetMetric(_wx.SYS_CAPTION_Y) + \
            _wx.SystemSettings.GetMetric(_wx.SYS_MENU_Y)
    _dy = _wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_Y) + \
            _wx.SystemSettings.GetMetric(_wx.SYS_CAPTION_Y) + \
            _wx.SystemSettings.GetMetric(_wx.SYS_MENU_Y)
window.dw = _dw
if _plat == 'Unix': window.dh = 76
else: window.dh = _dh
