...
EVENT_NAMES = ("click", "mousedown", "mousemove", "mouseup", "keydown", "keyup")
...
## mouse event codes: http://docs.wxwidgets.org/2.8.4/wx_wxmouseevent.html#wxmouseevent
    ...
    def _OnLeftMouseDown(self, evt):
        ...
        self._dispatch_event('mousedown', evt)
        evt.Skip() # to permit setting focus

    def _OnLeftMouseUp(self, evt):
        self._mt.leftUp()
        self._report_mouse_state(evt)
        self._dispatch_event('click', evt)
        self._dispatch_event('mouseup', evt)
...
##        print(evt.GetWheelRotation(), evt.GetWheelDelta())
    ...
    def _OnMouseMotion(self, evt):
        self._report_mouse_state(evt)
        self._dispatch_event('mousemove', evt)
        evt.Skip() # to permit setting focus
...
##                self.window.win.WarpPointer(self._cursorx, self._cursory)
    ...
    def _OnCharEvent(self, evt):
        ...
        self.pushkey(k)
        self._dispatch_event('keyup', {'evt':evt, 'key':k})
        evt.Skip()
    ...
    def _OnKeyDown(self, evt):
        k = self._ProcessKey(evt)    
        self._dispatch_event('keydown', {'evt':evt, 'key':k})
...
from graph import checkGraphMouse as _cgm
from controls import checkControlsMouse as _ccm
