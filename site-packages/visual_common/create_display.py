...
class window():
    def __init__(self, _make_panel=True, x=0, y=0, width=600, height=600, title='VPython'):
        ...
        if self._make_panel:
            # Must not create a panel or menu bar if user program does not call window
            self.panel = _wx.Panel(self.win)

        if self._make_panel or _plat == 'Unix':
            # If no menubar on Linux, there's a black void there
    ...
    def _OnMove(self, evt):
        ...
        if c is not None:
            ...
            if _plat == 'Windows':
                ...
                if self._make_panel: c._y -= _wx.SystemSettings.GetMetric(_wx.SYS_MENU_Y)
            c._width = c.width
            c._height = c.height
            c._report_resize()
        _Interact()
...
class display(cvisual.display_kernel):
        ...
        for enabled, fn, args, kw in self._bindings.get(name,[]):
            if enabled:
                ...
                if argspec.keywords is not None:
                    ...
                    [applyKW.update({name:newKW.get(name)}) for name in argspec.args if name is not 'self' and newKW.has_key(name)] # populate the available keywords
                
                try:
                    apply(fn, applyArgs, applyKW)  # call the function!
                except:
                    import traceback
                    traceback.print_exc()
    ...
    def trigger(self, namestring, *args, **kw):
        ...
        for name in namestring.split():
            ...
            apply(self._dispatch_event, (name,), kw)
    ...
    for d in displays:
        ...
        if _mouse_binding_names.intersection(events):
            ...
            while d.mouse.events:
                ...
                #
                # add 'event' attribute to the 'event' object (weird, I know)
                # also change 'event' property of d.mouse to match
                #
                if event.press:
                    event.event = d.mouse.event = "mousedown"
                    d._dispatch_event("mousedown", event)
                elif event.release:
                    event.event = d.mouse.event = "mouseup"
                    d._dispatch_event("mouseup", event)
                if event.click:
                    event.event = d.mouse.event = "click"
...
window.dw = _dw
if _plat == 'Unix': window.dh = 50
else: window.dh = _dh
if _plat == 'Macintosh':
    window.mh = _mh = 0
elif _plat == 'Windows':
    window.mh = _mh = _wx.SystemSettings.GetMetric(_wx.SYS_MENU_Y)
else:
    window.mh = _mh = 27
