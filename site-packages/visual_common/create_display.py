...
def _ManualLoop(manualIfEventLoopExists=False):

    global _evtloop

    if _evtloop is None and not manualIfEventLoopExists:
...
def _Interact(manualIfEventLoopExists=False):
    _BeforeManualLoop()
    _ManualLoop(manualIfEventLoopExists)
    _AfterManualLoop()
...
def sleep(dt):
    tend = _clock() + dt
    _Interact(True) # make sure we call _Interact at least once
    while _clock() < tend:
        rate(30)
