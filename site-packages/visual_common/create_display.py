...
class window():
    def __init__(self, menus=False, _make_panel=True, x=0, y=0, width=600, height=600, title='VPython'):
        global _do_loop
		...
		# If the canvas fills the screen (user did not create window object), do not
		# make a wx.Panel, which would prevent resizing the canvas with the window.
...
if _plat == 'Macintosh':
    ...
    _mh = _wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_Y) + \
            _wx.SystemSettings.GetMetric(_wx.SYS_CAPTION_Y)
