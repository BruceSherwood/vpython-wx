...
# Temporarily commented out label-related machinery:
# Mac makefile, vis/primitives.py, vis/create_display.py; don't import filedialog here

# Variables starting with "_" are not accessible to the importing program

# When running from a terminal on Mac, got a warning:
...
### Text routine for the text_to_bitmap function
##bitmap = text_to_bitmap('VPython', height=36, foreground=color.red, background=color.green,
##                   font='sans', style='italic', weight='normal')
##T = materials.texture(data=bitmap, mipmap=False, mapping='sign')
##box(material=T, axis=(0,0,1))

def text_to_bitmap(text, foreground=(0,0,0), background=(1,1,1),
                   height=13, font='sans',
                   style='normal', weight=_wx.FONTWEIGHT_NORMAL):
    ...
    w,h = _dc.GetTextExtent(text)
    h += 1 # leave a blank line of pixels under the text
    bmp = _wx.EmptyBitmap(w,h)
    _dc.SelectObject(bmp)
    _dc.SetTextForeground((255*foreground[0],255*foreground[1],255*foreground[2]))
    brush = _wx.Brush((255*background[0],255*background[1],255*background[2]))
    _dc.SetBackground(brush)
    _dc.Clear()
    _dc.DrawText(text, 0, 0)
    _dc.SelectObject( _wx.NullBitmap )
    img = _wx.ImageFromBitmap(bmp)
    arr = _numpy.fromstring(img.GetData(), dtype=_numpy.uint8).reshape((h,w,3))
    return arr
...
class display(cvisual.display_kernel, glcanvas.GLCanvas):
    ...
    def __init__(self, **keywords):
        ...
        self._leftdown = self._rightdown = self._middledown = 0
        self._cursorx = self._cursory = 0
        self._spinning = self._zooming = 0
...
## mouse event codes: http://docs.wxwidgets.org/2.8.4/wx_wxmouseevent.html#wxmouseevent
    def _OnLeftMouseDown(self, evt):
        self._leftdown = 1
        lock = 0
        if (self._rightdown or evt.ControlDown() or evt.AltDown()) and (self.userzoom or self.userspin):
            lock = 1
            self.window.win.SetCursor(_wx.StockCursor(_wx.CURSOR_BLANK))
            self._cursorx, self._cursory = evt.GetPosition()
            if (self._rightdown or evt.AltDown()) and self.userzoom:
                if not self._rightdown: self.CaptureMouse() # can drag outside of our window
                self._zooming = 1
                self._spinning = 0
                self._leftdown = 0
                self._middledown = 1
            elif evt.ControlDown() and self.userspin:
                self.CaptureMouse() # can drag outside of our window
                self._zooming = 0
                self._spinning = 1
                self._leftdown = 0
                self._rightdown = 1
        self._report_mouse_state(evt, lock)
    ...
    def _OnRightMouseUp(self, evt):
        ...
        if self._spinning or self._zooming: # reset mouse to original location
            ...
            self._spinning = self._zooming = 0
        self.ReleaseMouse()
        self._report_mouse_state(evt, 0)
    ...
    def _OnMiddleMouseUp(self, evt):
        self._middledown = 0
        if self._zooming: # reset mouse to original location
            self.window.win.WarpPointer(self._cursorx, self._cursory)
            self.window.win.SetCursor(_wx.StockCursor(_wx.CURSOR_ARROW))
            self._zooming = 0
            if self._rightdown: self._spinning = 1
        self._report_mouse_state(evt, 0)
        self.ReleaseMouse()
...
# CMD  + 1-button mouse = CTRL and CMD, left button
    ...
    def _report_mouse_state(self, evt, lock): # wx gives x,y relative to upper left corner
        x, y = evt.GetPosition()
        left = self._leftdown
        right = self._rightdown
        middle = self._middledown
        shift = evt.ShiftDown()
