...
class window(object):
    def __init__(self, menus=False, _make_panel=True, x=0, y=0, width=600, height=600,
                 title='VPython', visible=True, fullscreen=False, **keywords):
        global _do_loop
...
##        if _plat == 'Unix': height -= 25 # correction for Xubuntu; not understood
    ...
    def _OnExitApp(self, evt):
        ...
        if _displays.window_num == 0:
            ...
            pass # should remove all displays associated with this window from the displays list
...
##            apply(self._dispatch_event, (name,), kw)  # apply doesn't exist in Python 3
    ...
    def _make_canvas(self, parent):
        ...
        if _plat == 'Unix':
            y = 0
            h = h
        if not self.fillswindow: # place canvas in window already created by user
            x = self._x
            y = self._y
            w = self._width
            h = self._height
    ...
    def _create(self):
        ...
        if self.fillswindow: # the display fills the window; no panel created
            if self._visible is None: self._visible = True
            ...
            self.cursor = cursor(win=self.window.win)
        else:
