...
# sleep needs to call interact periodically during a long sleep
...
if 'idlelib' in _sys.modules:
    #
    # Looks like we're running inside IDLE. 
    #
    try:
        from idlelib import idlever as _idle_version_module
        _idle_version_number = _idle_version_module.IDLE_VERSION

        #
        # maybe check the version here in case IDLE get's fixed in the future?
        #
        
        print("It appears that you're executing this visual program in the IDLE environment")
        print("Be aware that this version of IDLE will not work for programs that do not")
        print("loop forever continually calling the visual 'rate' function.")
    except:
        import traceback as _tb
        _tb.print_exc()
        raise RuntimeError, "Cannot print warning message regarding running in IDLE"
...
class window(_wx.Frame):
    ...
    def _OnMove(self, evt):
        ...
        if _plat == 'Windows':
            self.disp._x -= _wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_X)
            self.disp._y -= _wx.SystemSettings.GetMetric(_wx.SYS_FRAMESIZE_Y) + \
