...
from numpy import array, zeros, arange, int32, float64, sin, cos, fromstring, uint8
from . import crayola
color = crayola
from math import pi
...
def text_to_bitmap(text, foreground=(0,0,0), background=(1,1,1),
    ...
    _dc.Clear()
    _dc.DrawText(text, 1, 0)
    _dc.SelectObject( _wx.NullBitmap )
    img = _wx.ImageFromBitmap(bmp)
    arr = fromstring(img.GetData(), dtype=uint8).reshape((h,w,3))
    return arr

class label (py_renderable, cvisual.label):
    def __init__(self, **keywords):
        # Need to extract text info to prevent constructor text
        # from driving set_bitmap prematurely:
        t = None
        if 'text' in keywords:
            t = keywords['text']
            del keywords['text']
        super(label, self).__init__(**keywords)
        if t != None:
            self._set_text(t)
    ...
    def _set_bitmap(self):
        ...
        if len(text) == 0:
            text = " "
        print('set_bitmap height=', self.height, self.color, self.background)
        bm = text_to_bitmap(text, self.color, self.background, 
                            self.height, "sans", "normal", "normal")
        h, w, d = bm.shape
        cvisual.label.set_bitmap(self, bm, w, h)
        
    def _set_text(self, text):
        cvisual.label.set_text(self, text)
        self._set_bitmap()
        
    text = property( cvisual.label.get_text, _set_text, None)
