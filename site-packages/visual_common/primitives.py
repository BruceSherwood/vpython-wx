...
def text_to_bitmap(text, color=(1,1,1), background=(0,0,0), opacity=1,
    ...
    if _dc == None:
        _dc = _wx.MemoryDC()
    height = int(height*23.0/30.0 + 0.5) # fudge factor for backwards compatibility
    ...
    return w, h, back, data
        
def get_bitmap(obj):
    text = obj.text
    if len(text) == 0:
        text = " "
    w, h, back, data = text_to_bitmap(text, obj.color, obj.background, obj.opacity,
