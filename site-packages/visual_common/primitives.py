...
def text_to_bitmap(text, color=(1,1,1), background=(0,0,0), opacity=1,
    ...
    _dc.SetTextForeground((255*color[0],255*color[1],255*color[2]))
    # Choose background of the text to match the appearance of
    # the transparent quad background displayed by label, to
    # improve the antialiasing of the text:
    back = (int(opacity*255*background[0]),
            int(opacity*255*background[1]),
            int(opacity*255*background[2]))
    ...
    temp = fromstring(img.GetData(), dtype=uint8).reshape((h,w,3))
    arr = zeros((h,w,4), uint8)
    b = array(back, uint8)
    for j in range(h):
        for i in range(w):
            byte = temp[j,i]
            arr[j,i][0:3] = byte 
            if (byte!=b).any(): arr[j,i][3] = 255
    return arr
...
def get_bitmap(obj):
    ...
    if len(text) == 0:
        text = " "
    arr = text_to_bitmap(text, obj.color, obj.background, obj.opacity,
                        obj.height, obj.font, "normal", "normal")
    h, w, d = arr.shape
    cvisual.label.set_bitmap(obj, arr, w, h)
