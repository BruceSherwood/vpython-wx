...
def checkGraphMouse(evt, gd):
    try:
        if gd.display.visible: gd.mouse(evt)
    except:
        pass
...
class gdisplay:
    def __init__(self, window = None, x=0, y=0, width=800, height=400,
        ...
        self.active = False
        if window: # graph in wxPython window thanks to Matt Greenwolfe
    ...
    def resize(self, x, y):
        ...
        if not self.active:
            ...
            self.display.bind("mousedown", checkGraphMouse, self)
            self.display.bind("mousemove", checkGraphMouse, self)
            self.display.bind("mouseup", checkGraphMouse, self)
