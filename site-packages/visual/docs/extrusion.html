...
<p class="Normal">The paths in the paths library all go counterclockwise in the xz plane, as seen from above. That is, if starting from a position on the x axis, they head in the -z direction and bend toward the left. If your shape has something sticking out on the right, it will be on the outer side of the extrusion, whereas something sticking out on the left will be on the inner side of the extrusion. </p>
                ...
                s = []<br />
                for p in pts:<br />
                &nbsp;&nbsp;&nbsp;&nbsp;s.append((p[0],-p[2])) # get the x and -z components</p>
                ...
                <p class="Normal">We create a list of 3D xyz points (with y = 0) in the xz plane, because that's what the paths library provides. We use paths.arc to obtain 3D points along a portion of a circle. In the &quot;for&quot; loop we create a list &quot;s&quot; consisting of 2D xy points, setting y to -z because the paths library objects head initially in the -z direction. We then convert the list of 2D xy points to a Polygon object and use it in the extrusion. After creating this Polygon object, we could of course combine it with other Polygon objects in the usual way to make holes in the object, etc.</p>
                <table width="699" border="0">
                  <tr>
