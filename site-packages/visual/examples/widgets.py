...
from visual import *
import wx # imports wxPython (see wxpython.org)

# wxPython is a Python library that makes it possible to create
# windows and handle events cross-platform, with native look-and-feel
...
# Among the documentation available at wxpython.org, a particularly
...
# wx.Button, having imported wx. Also, it is helpful to know that
...
w = window(width=2*L, height=L, title='Widgets')
...
t1 = wx.RadioBox(p, pos=(1.1*L,0.3*L), size=(0.2*L, 0.2*L),
                 choices = ['Red', 'Cyan'], style=wx.RA_SPECIFY_ROWS)
t1.Bind(wx.EVT_RADIOBOX, togglecubecolor)

wx.TextCtrl(p, pos=(1.4*L,100), value='You can type here:\n',
            size=(140,90), style=wx.TE_MULTILINE)
